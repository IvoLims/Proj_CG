...
Inicialmente, a esfera que representa um planeta foi gerada com raio 1 através do programa \emph{generator} desenvolvido na 1º Fase. Cada planeta é uma série de transformações geométricas aplicadas a essa esfera. As dimensões dos planetas não seguem a escala real pois tornar-se-ia dificil a sua visualização, também foram reduzidas as suas distâncias.\newline

\begin{minipage}{0.9\linewidth}
        \centering
		\includegraphics[width=\textwidth]{un.jpeg}\par\vspace{1cm}
                {\scshape\large Figura 2.1: Sistema Solar} \par
\end{minipage}
