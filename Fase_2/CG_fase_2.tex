...
A única parte que sofreu alterações foi a \emph{engine} para que fosse possível processar os novos elementos propostos pela segunda fase do trabalho.

\chapter{Atualização da \emph{engine}}
\section{Substituição da \emph{void draw()}}

Visto que temos uma nova estrutura do ficheiro \emph{xml} foi necessário repensar e reestruturar a função de carregamento do ficheiro de configuração (no nosso caso a \emph{void draw()}). Utilizamos primordialmente o seguinte exemplo:
...
Tendo este exemplo em mente e outros que foram disponibilizados no enunciado do trabalho, percebemos que a \emph{scene} tem um ou vários "filhos" \emph{group} e por sua vez cada \emph{group} possui um conjunto de transformações aplicáveis a todos os elementos dentro do nodo \emph{models}.

Depois desta breve análise chegamos a uma concluão que foi partir e dividir as funcionalidades do método feito na 1º fase, o mesmo originou uma \emph{readXML} e uma \emph{readXMLaux}.

\section{Função \emph{readXML}}

blah.

\section{Função \emph{void readXMLaux}}
